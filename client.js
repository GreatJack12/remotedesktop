...
// Strip wss:// prefix for display in the short input
const savedHost = localStorage.getItem('rd_host') || '';
hostInput.value = savedHost.replace(/^wss?:\/\//, '');
secretInput.value = localStorage.getItem('rd_secret') || 'fr0gz123';

// Check URL hash for host — e.g. https://yourpage.github.io/#wss://xxx.trycloudflare.com
if (window.location.hash) {
  const hashHost = decodeURIComponent(window.location.hash.slice(1));
  if (hashHost.startsWith('ws')) {
    hostInput.value = hashHost;
    localStorage.setItem('rd_host', hashHost);
  }
}

// Auto-connect if we have saved credentials
if (hostInput.value && secretInput.value) {
...
// Auto-connect when URL is pasted
hostInput.addEventListener('paste', () => setTimeout(connect, 100));

function connect() {
  const rawHost = hostInput.value.trim();
  const secret = secretInput.value.trim();
  if (!rawHost) { setStatus('Enter a host address'); return; }
  if (!secret) { setStatus('Enter your secret'); return; }

  // Prepend wss:// if not already there
  const host = rawHost.startsWith('ws') ? rawHost : `wss://${rawHost}`;

  localStorage.setItem('rd_host', rawHost);
    ...
    if (typeof e.data === 'string') {
      try {
        ...
        if (msg.type === 'info') {
          ...
          // Store real screen size for mouse coordinate mapping
          ...
          console.log('Got info:', msg);
        } else if (msg.type === 'frame') {
          // Always replace pending frame with latest — never queue
...
function getCanvasPos(e) {
  ...
  // Map display position to real screen coordinates
...
function send(obj) {
  if (ws && ws.readyState === WebSocket.OPEN) {
    // Don't send input if socket is backed up
